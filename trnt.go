...
func AllTorrents() map[int]*TRNT {
	...
	*/
	...
	//make map of torrents in database
	m := make(map[int]*TRNT)
	...
	for rows.Next() {
		...
		tor := new(TRNT)
		tor.AppendTorrent(id, size_sent, size_total, filename, from, to, created_at, updated_at)
		m[id] = tor
	}
	return m
}

func (tor *TRNT) AppendTorrent(id, size_sent, size_total int, filename, from, to string, created_at, updated_at []byte) error {
	tor.id = id
